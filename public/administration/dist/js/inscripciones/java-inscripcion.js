            ...
            function(isConfirm){
                if (isConfirm) {
                    ...
                    $.post(url, data, function (result) {
                        ...
                        swal("Error!! Inscripción no eliminado!");
                        row.show('slow');
                    });

                } else {
                    swal("Cancelado", "La inscripción no fue eliminada", "error");
                }
            });
    });
...
$("#usuarios").change(function (event) {
    ...
    if(cod==''){
        ...
        $.get(ruta+'/eventosNoMatriculados/' + event.target.value + "", function (response, facultades) {
            $("#eventos").empty();
...
$("#usuarios-validar").change(function (event) {
    var cod = document.getElementById("usuarios-validar").value;
    var ruta = document.getElementById("ruta").value;
    if(cod==''){
        $("#eventos").empty();
        $("#eventos").append("<option value=''>Seleccione el evento</option>")
        ...
        $.get(ruta+'/eventosMatriculados/' + event.target.value + "", function (response, facultades) {
            $("#eventos").empty();
            $("#eventos").append("<option value='' disabled selected>Seleccione el evento</option>")
            for (i = 0; i < response.length; i++) {
                $("#eventos").append("<option value='" + response[i].id + "'>" + response[i].nombre + "</option>")
